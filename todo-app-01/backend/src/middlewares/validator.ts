...
export const validateRegister = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!email.includes('@')) {
    sendError(res, 400, 'Format email tidak valid!');
    return;
  }
...
export const validateLogin = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!username || !password) {
    sendError(res, 400, 'Username dan password wajib diisi!');
    return;
  }
...
export const validateTodo = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!task || typeof task !== 'string') {
    sendError(res, 400, 'Task wajib diisi dengan format string!');
    return;
  }
